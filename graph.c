...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for(int i = 0; i < rows; i++) { 
        ...
        // adding the next node / attatched vertex to the current row. 
	    if(adj_mat[i][j] == 1) {
            adj_node_t *tmpnext = create_node(j);
            add_node(myList, i, tmpnext);
	    }
        }
    }
         ...
         int* color, int* distance, int* parent)
{
    ...
    // why do we have to initialize the vertices if they are already in list?

    // initializing the values of each vertex
    for (int i = 0; i < rows; i++) {
        color[i] = 0;
        distance[i] = -1; 
        parent[i] = -1;
    ...
    adj_node_t *src = create_node(source);
    distance[source] = 0; 
    parent[source] = -1;
    color[source] = 1;
    ...
    // making a "queue" by using the first row of the linked list 
    adj_node_t **queue = NULL;
    init_adj_list(&queue, 1);
    add_node(queue, 0, src); 
    ...
    adj_node_t *queptr = queue[0]; 
    
    // looping through source's neighbors and then its neighbors and so forth
    while(*queue != NULL) {  
        // removing from queue 
        int rem = remove_node(queue);
        adj_node_t *temp1 = list[rem];

        // finding the neighbors of rem
        while(temp1 != NULL) {
            int adjvert = temp1->vid;
            // if the neighbors haven't been visited prior
            if(color[adjvert] == 0) {
                color[adjvert] = 1;
                parent[adjvert] = rem;
                distance[adjvert] = distance[rem] + 1;
                adj_node_t *temp3 = create_node(adjvert); 
                add_node(queue, 0, temp3);
            }
            temp1 = temp1->next;
        }
        color[rem] = 2;

        // cases where there is only one item in queue
        if(queptr->next == NULL) { 
        }
